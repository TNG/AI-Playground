...
**Service lifecycle**: `getServices`, `startService`, `stopService`, `setUpService`, `serviceSetUpProgress` (M→R), `uninstall`, `updateServiceSettings`, `detectDevices`, `selectDevice`, `ensureBackendReadiness` (optional 7th `{ remember?: boolean }`; Home Agent summarizer passes `remember: false`), `setLastChatBackendLoadActive`
