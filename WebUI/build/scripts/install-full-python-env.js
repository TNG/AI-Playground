...
if (!envDirArg || !platformArg || !comfyUIDIrArg) {
  console.error(
    'Usage: node install-full-python-env.js --env_dir=$DIR ---platform=arc|ultra|ultra2\n',
  )
  process.exit(1)
}
...
function runPipInstall(pythonExe, requirementsFilePath) {
  ...
  const pipInstall = childProcess.spawnSync(pythonExe, [
    '-m',
    'pip',
    'install',
    '-r',
...
function main() {
  ...
  const platformSpecificRequirementsTxt = existingFileOrExit(
    path.join(__dirname, '..', '..', '..', 'service', `requirements-${platform}.txt`),
  )
  const requirementsTxt = existingFileOrExit(
    path.join(__dirname, '..', '..', '..', 'service', `requirements.txt`),
  )
  const comfyUiRequirementsTxt = existingFileOrExit(path.join(comfyUIDIr, `requirements.txt`))
  const ggufCostomNoderequirementsTxt = existingFileOrExit(
    path.join(comfyUIDIr, 'custom_nodes', 'ComfyUI-GGUF', `requirements.txt`),
  )
