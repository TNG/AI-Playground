...
const GAME_STUDIO_SKILL: SkillSource = {
  ...
  body: [
    ...
    '   leaves a named game.',
    '3. Decide the mechanic in one sentence: what the player does, how they win, how they lose.',
    '   Keep the first version small enough to finish.',
    '4. Build it with `edit` calls that replace one `// === section ===` block at a time — the',
    '   markers make the old text easy to match exactly. Start with state and update (the',
    '   mechanic), then draw. Never re-`write` the whole file: each reply has an output token',
    '   limit, and a call that exceeds it is rejected, costing you the attempt.',
