...
export function createPhisonKmRag(deps: PhisonKmRagDeps) {
  ...
  // Phison KM minimum (16 384) as a safety net when KM is active. Merged into a
  // single clamp (rather than two independent `if`/`else if` branches that used to
  // fight each other when maxContextSizeFromModel < 16 384) — kmContextFloorReachable
  // guarantees lo <= hi whenever both bounds apply, so this can never loop.
  watch(
    ...
    (newValue) => {
      ...
      if (!isAutoAdjustingContextSize && ragMode.value === 'phisonKm') {
        stashedStandardContextSize.value = null
      }
