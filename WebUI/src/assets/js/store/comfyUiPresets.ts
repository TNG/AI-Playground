...
export const useComfyUiPresets = defineStore(
  ...
  () => {
    ...
    watchEffect((onCleanup) => {
      if (comfyPort.value && comfyUiState.value?.status === 'running') {
        connectToComfyUi()
      } else {
        // Backend not running, close websocket if open
        if (websocket.value) {
          console.info('[ComfyUI] Backend not running, closing websocket')
          websocket.value.close()
          websocket.value = null
        }
      }
    ...
    async function generate(imageIds: string[], mode: WorkflowModeType, sourceImage?: string) {
      ...
      // Wait for websocket to be connected if backend is running but websocket isn't ready yet
