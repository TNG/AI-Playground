...
export const useQwen3TextToSpeech = defineStore('qwen3TextToSpeech', () => {
  ...
  // Step 8 (§6.1): the default voice and the saved voice list are kernel-owned
  // preferences (preferences.json), hydrated before mount.
  const prefs = makeFileBackedPreference({
    section: 'qwen3TextToSpeech',
    refs: {
      defaultSpeaker,
      defaultLanguage,
      defaultMode,
  ...
  async function ensureModelLoaded(mode?: Qwen3TtsSynthesisMode): Promise<void> {
    ...
    if (!response.ok || payload.code !== 0) {
      throw new Error(payload.message ?? `Failed to load Text To Speech model (${response.status})`)
    }
  }
  ...
  function isBackendSetUp(): boolean {
    return backendServices.info.find((s) => s.serviceName === 'qwen3-tts-backend')?.isSetUp === true
  }
