        ...
        <div class="flex flex-col gap-2">
          ...
          <RadioGroup v-model="transport" class="flex gap-4">
            ...
            <div class="flex items-center gap-2 opacity-50">
              <RadioGroupItem id="transport-http" value="http" disabled />
              <Label for="transport-http" class="cursor-not-allowed">http</Label>
            </div>
          </RadioGroup>
        </div>
...
async function handleAdd() {
  ...
  if (transport.value === 'http') {
    errorMessage.value = 'HTTP transport is not yet supported'
    return
  }
