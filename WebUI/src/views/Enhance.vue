  ...
  <div class="flex flex-col h-full p-4">
    <div class="image-panel flex-auto flex pb-4 h-0">
      <div class="image-preview-panel flex-none overflow-y-auto">
        <div
          ...
          :class="{ active: previewIdx == -1 }"
          @click="swithPreview(-1)"
        >
      ...
      <div class="flex-auto relative flex items-center justify-center">
        ...
        >
          ...
          <div class="relative">
            ...
            />
            <inpaint-mask
              ref="inpaintMaskCompt"
              v-show="mode == 3 && previewIdx == -1 && generateState != 'no_start'"
              :brush-size="maskBrush.size"
              :easer-size="maskEaser.size"
            ...
            >
              <div class="flex flex-col items-center justify-center gap-1 flex-auto">
                ...
                <vertical-slide-bar
                  type="vertical"
                  :showEditbox="false"
                  v-model:current="maskBrush.size"
                  :min="1"
                  :max="64"
                  :step="1"
                  class="flex-auto"
            ...
            >
              <div class="flex flex-col items-center justify-center gap-1 flex-auto">
                ...
                <vertical-slide-bar
                  type="vertical"
                  :showEditbox="false"
                  v-model:current="maskEaser.size"
                  :min="1"
                  :max="64"
                  :step="1"
                  class="flex-auto"
    ...
    <div class="flex flex-col flex-none h-44">
      <div class="enhance-tabs flex-none pt-2 flex items-end justify-start gap-1 text-gray-400">
        ...
        >
          <span>{{ languages.ENHANCE_UPSCALE }}</span>
        </button>
        <button
          class="tab"
          :class="{ active: mode == 2 }"
          @click="switchFeature(2)"
          :title="languages.ENHANCE_IMAGE_PROMPT_TIP"
        >
          <span>{{ languages.ENHANCE_IMAGE_PROMPT }}</span>
        </button>
        <button
          class="tab"
          :class="{ active: mode == 3 }"
          @click="switchFeature(3)"
          :title="languages.ENHANCE_INPAINT_TIP"
        >
          <span>{{ languages.ENHANCE_INPAINT }}</span>
        </button>
        <button
          class="tab"
          :class="{ active: mode == 4 }"
          @click="switchFeature(4)"
          :title="languages.ENHANCE_OUTPAINT_TIP"
        >
      ...
      <div class="enhance-content flex-auto border p-2 rounded-b-md border-color-spilter">
        <div class="w-80 rounded-lg bg-color-control-bg relative h-full">
          ...
          <input
            ...
            :title="languages.COM_UPLOAD_TIP_4"
            @change="chooseImage"
            @drop="dropImageFile"
          />
        </div>
...
async function generate() {
  if (imageGeneration.activeWorkflow.backend === 'comfyui') {
    toast.error(
      '"Enhance" does not support Workflow mode at the moment. Please go to settings and change from Workflows to Default modes.',
    )
    return
  }
  ...
  try {
    ...
    const model_repo_id =
      ...
      imageGeneration.inpaintModel != i18nState.ENHANCE_INPAINT_USE_IMAGE_MODEL
...
async function checkModel() {
  return new Promise<void>(async (resolve, reject) => {
    ...
    if (
      ...
      checkList.push({
        repo_id: imageGeneration.inpaintModel,
        type: Const.MODEL_TYPE_INPAINT,
        backend: imageGeneration.activeWorkflow.backend,
      })
    } else {
      checkList.push({
        repo_id: imageGeneration.imageModel,
    ...
    if ([1, 3, 4].includes(mode.value)) {
      checkList.push({
        repo_id: 'RealESRGAN_x2plus',
        type: Const.MODEL_TYPE_ESRGAN,
        backend: imageGeneration.activeWorkflow.backend,
      })
    }
    if (imageGeneration.lora != 'None') {
      checkList.push({
        repo_id: imageGeneration.lora,
