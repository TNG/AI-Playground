...
#   ./containers/run.sh              Desktop mode — Electron window via X11 (default)
#   ./containers/run.sh --headless   Headless mode — web server on http://localhost:8080
...
#
# Mode selection:
#   Desktop (default): Electron window is forwarded to your host desktop via X11.
#     Requires a running X11 display. Works like the baremetal install.
#   Headless:          No display needed. Open http://localhost:8080 in any browser.
#     Useful for servers, VMs, CI, or remote access. Same backends, same GPU support.
#
# Under the hood this is a thin wrapper around `docker compose`.
# To use docker compose directly after run.sh has generated .env:
...
# ── Subcommands ────────────────────────────────────────────────────────────────
# Looped over all args (not just $1) so flags can combine, e.g. `--headless --build`.
for arg in "$@"; do
  case "$arg" in
    --logs)
...
# repositories.intel.com (Intel GPU/NPU driver packages, fetched during the
# image build) is a public internet host and must be reached through the proxy
# like any other. A common corporate no_proxy blanket-excludes *.intel.com for
# internal services, which also (incorrectly) bypasses the proxy for this
# public repo and hangs/fails the build. Strip that pattern only for the
# build-time proxy args — the running container's own NO_PROXY (below) is left
# exactly as detected, so runtime behavior is unchanged.
BUILD_NO_PROXY_VAL=$(echo "$NO_PROXY_VAL" | sed -E 's/(^|,)\.?intel\.com(,|$)/\1/g; s/,,+/,/g; s/^,//; s/,$//')
if [ "$BUILD_NO_PROXY_VAL" != "$NO_PROXY_VAL" ]; then
    warn "Build proxy: excluding .intel.com from no_proxy so repositories.intel.com is reached via proxy"
fi

# ── 4b. Corporate CA certificate (optional) ───────────────────────────────────
# If the app's own downloads (e.g. llama.cpp from GitHub) fail inside the
# container with ERR_CERT_AUTHORITY_INVALID, your network TLS-intercepts
# HTTPS with an internal CA. Point AIPG_EXTRA_CA_CERT at that CA's PEM/CRT
# file (or we'll look for it in the usual env vars other tools use) and it
# will be trusted inside the image. Empty by default — safe no-op otherwise.
CA_CERT_SRC="${AIPG_EXTRA_CA_CERT:-${NODE_EXTRA_CA_CERTS:-${SSL_CERT_FILE:-${REQUESTS_CA_BUNDLE:-${CURL_CA_BUNDLE:-}}}}}"
mkdir -p "$SCRIPT_DIR/ca-certs"
rm -f "$SCRIPT_DIR/ca-certs"/*.crt
if [ -n "$CA_CERT_SRC" ] && [ -f "$CA_CERT_SRC" ]; then
    cp "$CA_CERT_SRC" "$SCRIPT_DIR/ca-certs/extra-ca.crt"
    ok "Extra CA certificate: $CA_CERT_SRC (will be trusted in-container)"
fi
...
AIPG_HEADLESS=$( [ "$HEADLESS" = true ] && echo 1 || echo 0 )
...
NO_PROXY=${NO_PROXY_VAL}
BUILD_NO_PROXY=${BUILD_NO_PROXY_VAL}
...
if [ "$HEADLESS" = true ]; then
    ok "Mode: headless — open http://localhost:8080 in your browser"
else
    ok "Mode: desktop — Electron window on display $DETECTED_DISPLAY"
fi
echo "  Image:       $IMAGE"
