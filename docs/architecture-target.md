...
- **Remaining §6 buckets** (for the next slices): the per-preset knobs half of
  ...
  (`comfyUiParameters` / `llamaCppParameters`) → `settings.json`, `ragList` → the RAG bucket,
  `lastSelectedDeviceIdPerBackend` dedupe, workspace last-used files.
