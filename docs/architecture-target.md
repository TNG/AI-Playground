...
**Status: steps 1–8 of the migration order (§8) have nine slices landed (conversations,
agent-session files, generated-media records, user preferences, per-preset settings knobs,
backend launch flags, the RAG document list, the agent workspace state, and the last-used
preset names as kernel-owned stores, `userSelectedMode` deleted); the §6 bucket list is
