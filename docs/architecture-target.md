...
**Status: steps 1–8 of the migration order (§8) have seven slices landed (conversations,
agent-session files, generated-media records, user preferences, per-preset settings knobs,
backend launch flags, and the RAG document list as kernel-owned stores, `userSelectedMode`
...
`preferences.json` and `rag/documents.json` — written by
`electron/conversations/conversationFiles.ts`,
`electron/agentMode/agentSessionFiles.ts`, `electron/media/mediaItemFiles.ts`,
`electron/preferences/preferencesFile.ts` and `electron/rag/ragDocumentFiles.ts`. The
agent-session index also carries `activeSessionId`; Pi's own session files are untouched.)
...
- **Remaining §6 buckets** (for the next slices): workspace last-used files, the
  `defaultPreset` preference. The launch-flags / device-dedupe bucket landed with the
  sixth slice, the RAG document list with the seventh (row 8).
