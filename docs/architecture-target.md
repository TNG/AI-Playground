...
**Status: steps 1–8 of the migration order (§8) have eight slices landed (conversations,
agent-session files, generated-media records, user preferences, per-preset settings knobs,
backend launch flags, the RAG document list, and the agent workspace state as kernel-owned
stores, `userSelectedMode` deleted); remaining §6 buckets are incremental.**
...
- **Remaining §6 buckets** (for the next slices): the `defaultPreset` preference. The
  launch-flags / device-dedupe bucket landed with the sixth slice, the RAG document list
  with the seventh, the agent workspace pointers with the eighth (row 8).
