...
def torch_bmm_32_bit(input, mat2, *, out=None):
    ...
    (
        ...
        split_2_slice_size,
        split_3_slice_size,
    ) = find_bmm_slice_sizes(input.shape, input.element_size(), mat2.shape)
...
def scaled_dot_product_attention_32_bit(
    ...
    (
        ...
        split_2_slice_size,
        split_3_slice_size,
    ) = find_sdpa_slice_sizes(query.shape, query.element_size())

    # Slice SDPA
    if do_split:
        batch_size_attention, query_tokens, shape_three = (
            query.shape[0],
            query.shape[1],
            query.shape[2],
        )
