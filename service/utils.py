...
def get_model_path(type: str, backend: str):
    ...
    match backend:
        case "default":
            ...
            logging.warning(
                f"Default backend (ipexllm) is no longer supported. Cannot get model path for type {type}."
            )
            return None
        case "llama_cpp":
...
def check_mmodel_exist(
    type: str, repo_id: str, backend: str, model_path: str | None = None
) -> bool:
...
def check_model_exists_with_path(type: str, repo_id: str, model_path: str) -> bool:
    ...
    if type == "faceswap" or type == "facerestore":
        ...
        if not os.path.isfile(comfy_ui_path):
            logging.info(
                f"Restoring {type} model {repo_id} from storage to ComfyUI directory"
            )
        ...
        dir_to_look_for = os.path.join(
            model_path, "vit-base-nsfw-detector", extract_model_id_pathsegments(repo_id)
        )
    ...
    if type == "faceswap" or type == "facerestore":
        ...
        if os.path.isfile(storage_path):
            ...
            if copy_faceswap_facerestore_to_comfyui(type, repo_id):
                ...
                logging.warning(
                    f"Model exists in storage but failed to copy to ComfyUI: {repo_id}"
                )
        ...
        dir_to_look_for = os.path.join(
            config.comfy_ui_model_paths.get(type),
...
def check_defaultbackend_mmodel_exist(type: str, repo_id: str) -> bool:
    ...
    logging.warning(
        f"Default backend (ipexllm) is no longer supported. Model {repo_id} of type {type} cannot be checked."
    )
    return False
...
def copy_faceswap_facerestore_to_comfyui(
    type: str, repo_id: str, storage_model_path: str | None = None
) -> bool:
    """
    ...
    if type not in ("faceswap", "facerestore"):
        logging.warning(
            f"copy_faceswap_facerestore_to_comfyui called for unsupported type: {type}"
        )
        return False
    ...
    try:
        ...
        if not os.path.exists(source_path):
            logging.warning(
                f"Source model not found at {source_path}, cannot copy to ComfyUI"
            )
            return False
